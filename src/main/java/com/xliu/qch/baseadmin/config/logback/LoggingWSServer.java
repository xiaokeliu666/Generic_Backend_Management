package com.xliu.qch.baseadmin.config.logback;
...
/**
 * Using WebSocket to retrieve the real time log
 */
@Slf4j
@Component
...
public class LoggingWSServer {
    ...
    /**
     * If connections built successfully
     */
    @OnOpen
    public void onOpen(Session session) {
        // add to the collection
        sessionMap.put(session.getId(), session);
        lengthMap.put(session.getId(), 1);// start from first line by default

        // get logging info
        new Thread(()->{
            log.info("LoggingWebSocketServer task start");
            boolean first = true;
