...
import com.xliu.qch.baseadmin.common.pojo.MonitorVo;
...
public class SystemMonitorUtil {
    ...
    static {
        monitorVo.setOs(System.getProperties().getProperty("os.name") + "   " +  System.getProperties().getProperty("os.arch"));// Operating system
        monitorVo.setCpuInfo(systemInfo.getHardware().getProcessor().getName());// CPU name
        monitorVo.setJvmJavaVersion(System.getProperty("java.version"));// Java version
        monitorVo.setRunTime(simpleDateFormat.format(ManagementFactory.getRuntimeMXBean().getStartTime()));// Application start time
    }
    ...
    /**
     * Get Windows disk usage rate
     *
     * @return disk usage rate
     */
